...
$current_user_id = isset($_GET['id']);

$rec_sql = "SELECT b.mmsID, b.Title, a.authorName, COUNT(bl.logIG) as popularity
            FROM books b
            JOIN author a ON b.authorID = a.authorID
            JOIN book_genre bg ON b.mmsID = bg.mmsID
            JOIN genres g ON bg.genreID = g.genreID
            JOIN users u ON (g.genreName = u.Genre1 OR u.genreName = u.Genre2 OR g.genreName = u.Genre3)
            LEFT JOIN book_log bl ON b.mmsID = bl.mmsID AND bl.Action = 'return'
            WHERE u.userID = ?
            GROUP BY b.mmsID
